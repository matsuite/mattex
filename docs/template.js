...
class Example extends Presentation {
  // Extend the Presentation class for an easier way to run the program
  constructor(img) {
    super(img);
  }
  async run() {
  ...
  async scene_A() {
    ...
    img.rect(
      ...
      5,
      5,
      new ColorDetails("#0000ff", "#000000"),
      new Properties(0.1)
    ); // rectangle at x:1,y:1, with a width and height of 5, a border of blue, filled with black, and a stroke width of 0.1 units
  }
}
var p; // Debugging purposes for if something is NaN or an error
const run = async () => {
  // async runner for async purposes
