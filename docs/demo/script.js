...
class OpenVS extends Presentation {
    ...
    async loadData() {
        try {
            this.coins = isNaN(parseInt(localStorage.getItem("coin"))) ? 0 : parseInt(localStorage.getItem("coin"));
            this.lvl = isNaN(parseInt(localStorage.getItem("lvl"))) ? 1 : parseInt(localStorage.getItem("lvl"));
            this.keystrokes = 1e+22;
            document.getElementById("pb").innerHTML = isNaN(parseInt(localStorage.getItem("keystrokes"))) ? 1e+22 : parseInt(localStorage.getItem("keystrokes"));
    ...
    async levelUp() {
        this.coins += 2;
        this.lvl++;
        if (this.keystrokes < (isNaN(parseInt(localStorage.getItem("keystrokes"))) ? 1e+22 : parseInt(localStorage.getItem("keystrokes")))) {
    ...
    async movement(evt) {
        ...
        try {
            ...
            if ((that.map["s"] || that.map["ArrowDown"]) && that.playerPos[1] + 2 <= that.yxGameMap.length) {
                evt.preventDefault();
                that.playerPos[1] = (that.playerPos[1] + that.speed / that.img.u);
                that.keystrokes++;
            }
    ...
    async renderPlayer() {
        const img = this.img;
        setGCTX(img);
        img.rect(this.playerPos[0], this.playerPos[1], 4 / 5, 4 / 5, new ColorDetails("#ff0000", "#ff0000"), new Properties(0));
    }
    async gameMapBg() {
        const that = this;
        try {
            ...
            for (var y = 0; y < that.yxGameMap.length; y++) {
                for (var x = 0; x < that.yxGameMap[y].length; x++) {
                    if (that.yxGameMap[y][x] == "G") {
                        img.rect(x, y, 1, 1, new ColorDetails("#001100", "#00aa00"), new Properties(0.25));
                    } else if (that.yxGameMap[y][x] == "E") {
                        img.rect(x, y, 1, 1, new ColorDetails("#110000", "#aa0000"), new Properties(0.25));
                    } else if (that.yxGameMap[y][x] == "C") {
                        img.rect(x, y, 1, 1, new ColorDetails("#111100", "#aaaa00"), new Properties(0.25));
                    }
                }
            }
        } catch (e) {}
    ...
    async checkSafety() {
        try {
            if (this.yxGameMap[this.playerPos[1]][this.playerPos[0]] == "E") {
                ...
                this.coins++;
                this.yxGameMap[this.playerPos[1]] = setCharAt(this.yxGameMap[this.playerPos[1]], this.playerPos[0], "G");
