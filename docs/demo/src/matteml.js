...
function parse(instr, ele, w = -1, h = -1) {
  ...
  var tok = instr.split("\n").filter((e) => e);
  var res = [
    `(async ()=>{m_package([media,mattexy,tex,example]);var img = new Image("${ele}", ${w}, ${h});setGCTX(img);`,
  ];
  var end = "})();";
  for (var i = 0; i < tok.length; i++) {
    ...
    switch (name) {
      ...
      case "mattex.circle":
        res.push(
          `img.circle(${args[0]}, ${args[1]}, ${args[2]}, new ColorDetails("${args[3]}", "${args[4]}"), new Properties("${args[5]}"));`
        );
        break;
      case "mattex.rect":
        res.push(
          `img.rect(${args[0]},${args[1]},${args[2]},${args[3]}, new ColorDetails("${args[4]}", "${args[5]}"), new Properties("${args[6]}"));`
        );
        break;
      case "media.image":
        res.push(
          `img.image(${args[0]},${args[1]},${args[2]},${args[3]},"${args[4]}")`
        );
        break;
      case "mattex.tex":
        res.push(
          `img.tex(${args[0]}, ${args[1]}, ${args[2]}, ${args[3]}, "${args[4]}")`
        );
      case "mattex.text":
        res.push(
          `img.text(${args[0]}, ${args[1]}, ${args[2]}, ${args[3]}, "${args[4]}")`
        );
      case "mattex.s.tex":
        res.push(`img.s_tex(${args[0]}, ${args[1]}, ${args[2]}, "${args[3]}")`);
      case "mattex.s.text":
        res.push(
          `img.s_text(${args[0]}, ${args[1]}, ${args[2]}, "${args[3]}")`
        );
