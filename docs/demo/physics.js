...
function terminalVelocity(m, g, rho, A, C_d) { // rho = density
...
function projMotion(d,v,m,rho,a,C_d,g,t) {
	...
	v_y = v.y-(g.x*t.x);
	y=d.y+(v.y*t.x)+((1/2)*g.x*(Math.pow(t.x,2)));
	return [new Vector2(Math.abs(v_x) > v_t.x ? (v_x < 0 ? -Math.abs(v_t.x) : Math.abs(v_t.x)) : v_x,
	 Math.abs(v_y) > v_t.x ? (v_y < 0 ? -Math.abs(v_t.x) : Math.abs(v_t.x)) : v_y
	 ), new Vector2(x, y), g, t];
}
...
function elasticCollision(d, d2, Vv, V2v, m, theta, phi) {
    ...
    var v = Vv;
	var vd_1x = ((V.x*Math.cos(theta.x-phi.x)*(m.x-m.y)+(2*m.y*v.y*Math.cos(theta.y-phi.x)))/(m.x+m.y))*Math.cos(phi.x)+v.x*Math.sin(theta.x-phi.x)*Math.cos(phi.x+Math.PI/2);
	var vd_1y = ((V.x*Math.cos(theta.x-phi.x)*(m.x-m.y)+(2*m.y*v.y*Math.cos(theta.y-phi.x)))/(m.x+m.y))*Math.sin(phi.x)+v.x*Math.sin(theta.x-phi.x)*Math.sin(phi.x+Math.PI/2);
    var vd_2x = ((V.y*Math.cos(theta.y-phi.x)*(m.y-m.x)+(2*m.x*v.x*Math.cos(theta.x-phi.x)))/(m.y+m.x))*Math.cos(phi.x)+v.y*Math.sin(theta.y-phi.x)*Math.cos(phi.x+Math.PI/2);
	var vd_2y = ((V.y*Math.cos(theta.y-phi.x)*(m.y-m.x)+(2*m.x*v.x*Math.cos(theta.x-phi.x)))/(m.y+m.x))*Math.sin(phi.x)+v.y*Math.sin(theta.y-phi.x)*Math.sin(phi.x+Math.PI/2);
    return [new Vector2(vd_1x, vd_1y), new Vector2(vd_2x, vd_2y)];
}
function didCollide(d, d2, s) {
    return (between(d.x, d2.x, d2.x+s.x) && between(d.y, d2.y, d2.y+s.x)) || (between(d2.x, d.x, d.x+s.x) && between(d2.y, d.y, d.y+s.x));
}
function sides(d,v) {
...
var physics = {
	...
	optionalRequires: [],
	optFuncs: {},
	description: "Adds support for basic rojectile physics in mattex.",
	funcs: {
		p_obj(x,y,w,h,m,prop=EARTH_SETTINGS) {
            return new sObject(new Vector1(m),new Vector2(w,h),new Vector2(x,y), prop);
        },
